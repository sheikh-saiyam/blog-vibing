  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <article>
        ...
        <div className="prose prose-sm max-w-none mb-12 whitespace-pre-wrap">{post.content}</div>
      </article>
      ...
      <div className="border-t pt-8">
          ...
          <Card className="p-4 mb-6">
            <div className="space-y-2">
              {replyingTo && (
                <p className="text-sm text-muted-foreground">
                  Replying to comment
                  <Button variant="ghost" size="sm" onClick={() => setReplyingTo(null)} className="ml-1 h-auto p-0">
                    (cancel)
                  </Button>
                </p>
              )}
              <Textarea
                ...
                rows={3}
              />
              <Button onClick={handleSubmitComment} disabled={!commentContent.trim() || createComment.isPending}>
        ...
        <div className="space-y-4">
          {rootComments.map((comment) => (
            <CommentItem key={comment.id} comment={comment} postId={postId} onReply={setReplyingTo} />
          ))}
        </div>
      </div>
    </div>
